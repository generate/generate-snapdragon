...
describe('generate-snapdragon', function() {
  ...
  describe('snapdragon (CLI)', function() {
    it('should run the default task using the `generate-snapdragon` name', function(cb) {
      if (isTravis) {
        this.skip();
        return;
      }
      app.use(generator);
      app.generate('generate-snapdragon', exists('index.js', cb));
    });
    ...
    it('should run the default task using the `generator` generator alias', function(cb) {
      ...
      app.use(generator);
      app.generate('snapdragon', exists('index.js', cb));
    });
  });

  describe('snapdragon (API)', function() {
    it('should run the default task on the generator', function(cb) {
      app.register('snapdragon', generator);
      app.generate('snapdragon', exists('index.js', cb));
    });

    it('should run the `parser` task', function(cb) {
      app.register('snapdragon', generator);
      app.generate('snapdragon:parser', exists('index.js', cb));
    });

    it('should run the `compiler` task', function(cb) {
      app.register('snapdragon', generator);
      app.generate('snapdragon:compiler', exists('index.js', cb));
    });

    it('should run the `default` task when defined explicitly', function(cb) {
      app.register('snapdragon', generator);
      app.generate('snapdragon:default', exists('index.js', cb));
    });
  });
  ...
  describe('sub-generator', function() {
    it('should work as a sub-generator', function(cb) {
      app.register('foo', function(foo) {
        foo.register('snapdragon', generator);
      });
      app.generate('foo.snapdragon', exists('index.js', cb));
    });
    ...
    it('should run the `generator:default` task when defined explicitly', function(cb) {
      ...
      app.generate('foo.snapdragon:default', exists('index.js', cb));
    });
    ...
    it('should run the `generator:compiler` task', function(cb) {
      ...
      app.generate('foo.snapdragon:compiler', exists('index.js', cb));
    });

    it('should run the `generator:parser` task', function(cb) {
      app.register('foo', function(foo) {
        foo.register('snapdragon', generator);
      });
      app.generate('foo.snapdragon:parser', exists('index.js', cb));
    });
    ...
    it('should run tasks as a sub-generator', function(cb) {
      ...
      app.generator('foo', function(sub) {
        sub.register('snapdragon', require('..'));
        sub.generate('snapdragon:unit-test', function(err) {
          if (err) return cb(err);
